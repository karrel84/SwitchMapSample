 ...
 */
class ImageAdapter(context: Context) : RecyclerView.Adapter<ImageAdapter.ImageHolder>() {
    ...
    override fun onBindViewHolder(holder: ImageHolder, position: Int) {
        items[position].let { url ->
            println("url : $url")
            Glide.with(holder.itemView.context).load(url).into(holder.itemView.image)
        }
    }
